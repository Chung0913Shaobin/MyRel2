...
openai.api_key = "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"
...
def ask_openai(prompt, role):
    try:
        ...
        if "choices" in response:
            ...
            logging.error(f"回傳格式不正確：{response}")
            return "OpenAI 回傳格式錯誤"
...
def upload_file():
    ...
    {resume_text}
    """
    categorized_skills = ask_openai(skill_prompt, "你是一位專家，可以提取履歷中的技能並分類。")
    ...
    簡明扼要地提供具體改進方向。
    """
    suggestions = ask_openai(suggestion_prompt, "你是一位專家，專門分析履歷結構並提出改進建議。")
   ...
   - 21-25：個人形象完整，極具競爭力
    ...
    關鍵字與技能優化建議：
    根據推薦的職缺，請提供履歷可增加或優化的關鍵字與技能，使履歷更符合市場需求。例如：
    - 建議補充技能：
        - [技能名稱]: {"該技能對應的職缺優勢"}
    - 建議加入關鍵字：
        - [關鍵字]: {"關鍵字對履歷吸引力的提升效果"}
    ...
    evaluation = ask_openai(evaluation_prompt, "你是一位專家，負責評估履歷的品質並給予評分。")
